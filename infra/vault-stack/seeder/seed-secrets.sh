...
echo "==> Logging in as cyber (attend vault-bootstrap si besoin)..."
until VAULT_TOKEN=$(curl -skf -X POST \
  -d "{\"password\":\"$CYBER_PASSWORD\"}" \
  "$VAULT_ADDR/v1/auth/userpass/login/cyber" | jq -re '.auth.client_token') 2>/dev/null; do
  sleep 2
done
