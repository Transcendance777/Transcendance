...
- l'architecture actuelle front/back/infra ;
...
`docker-compose.yaml` definit les services :
...
- `nginx`: entree publique en `8080:80` et `8443:443`, reverse proxy vers front/back.
...
Fichier : `backend/src/init/initPrisma.js`
...
Fichier : `backend/src/routes/games.js`
...
Fichier : `backend/src/routes/user.js`
...
Note : `SettingsPage.jsx` appelle aussi `/api/apikey`, mais aucune route `/api/apikey` n'est montee dans `backend/src/index.js`. Il faudra verifier si cette fonctionnalite est incomplete ou dans une autre branche.
...
Fichier : `front/src/components/InscriptionForm.jsx`
...
Fichier : `front/src/pages/HomePage.jsx`
...
9. Les generated Prisma files dans `backend/generated/prisma` semblent partiels/stales par rapport au schema actuel ; le code utilise `@prisma/client`, donc ce dossier n'est probablement pas la source active.
...
front/src/components/chat/ConversationList.jsx
front/src/components/chat/ChatWindow.jsx
front/src/components/chat/MessageBubble.jsx
front/src/components/chat/MessageComposer.jsx
front/src/components/chat/TypingIndicator.jsx
front/src/styles/ChatPage.css
```

### Integration routing

Dans `front/src/App.jsx` :
...
1. Installer `socket.io` dans `backend/package.json`.
...
4. Generer Prisma.
5. Remplacer `app.listen` par `http.createServer(app)` dans `backend/src/index.js`.
6. Configurer Socket.IO avec CORS adapte.

### Phase 2 - REST chat

1. Creer `backend/src/routes/chat.js`.
...
1. Ajouter `location /socket.io/` dans `nginx/nginx.conf`.
...
1. Creer `front/src/services/socket.js`.
2. Creer `front/src/services/chatApi.js`.
3. Ajouter `ChatPage`.
